...
if __name__ == "__main__":
    ...
    for idx, target_dir in enumerate(dirs):
        ...
        for section_idx, section_name in enumerate(unique_section_names):
            ...
            print(f"data_ea_section_{machine_type}_{section_name}: {data_ea_section.shape}")
            np.save(f"/run/media/viblab/Markov1/Bagus Adhi/bigan_dcase/data/train/x_train_{machine_type}_{section_name}.npy", data_ea_section)

            label = np.zeros((data_ea_section.shape[0]), float)
            print(f"label_{machine_type}_{section_name}: {label.shape}")
            np.save(f"/run/media/viblab/Markov1/Bagus Adhi/bigan_dcase/data/train/y_train_{machine_type}_{section_name}.npy", label)
        ...
        np.savetxt("/run/media/viblab/Markov1/Bagus Adhi/bigan_dcase/condition", condition, delimiter = ',')


        # 1D vector to 2D image
        data = data.reshape(data.shape[0], param["feature"]["n_frames"], param["feature"]["n_mels"], 1)
        print(f"data 3 : {data.shape}")

...
def train_and_test(nb_epochs, weight, method, degree, random_seed, label):
    ...
    with sv.managed_session() as sess:
        ...
        fill = np.ones([batch_size - size, 64, 128, 1])
        ...
        # scores = np.mean(np.log(np.maximum(1.0 - scores, sys.float_info.epsilon)) - np.log(np.maximum(scores, sys.float_info.epsilon)))

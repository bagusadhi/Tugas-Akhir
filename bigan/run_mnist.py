...
def train_and_test(nb_epochs, weight, method, degree, random_seed, label):
    ...
    # Variables
    #tf.disable_v2_behavior() 
    #input_pl = tf.Variable(tf.float32, shape=data.get_shape_input(), name="input")
    #is_training_pl = tf.Variable(tf.bool, [], name='is_training_pl')
    #learning_rate = tf.Variable(tf.float32, shape=(), name="lr_pl")
    ...
    with tf.compat.v1.variable_scope('discriminator_model'):
    ...
    with tf.name_scope('Testing'):
        with tf.compat.v1.variable_scope('Reconstruction_loss'):
            ...
            delta_flat = tf.compat.v1.estimator.layers.flatten(delta)

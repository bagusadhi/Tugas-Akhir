...
def encoder(x_inp, is_training=False, getter=None, reuse=False):
    ...
    with tf.compat.v1.variable_scope('encoder', reuse=reuse, custom_getter=getter):
        ...
        with tf.compat.v1.variable_scope(name_net):
            net = tf.compat.v1.keras.layers.conv2d(x_inp,
                                   32,
                                   [3, 3],
                                   padding='SAME',
        ...
        name_net = 'layer_2'
        with tf.compat.v1.variable_scope(name_net):
            net = tf.compat.v1.keras.layers.conv2d(net,
                                   64,
                                   [3, 3],
                                   padding='SAME',
                                   strides=[2, 2],
                                   kernel_initializer=init_kernel,
                                   name='conv')
            net = tf.compat.v1.keras.layers.batch_normalization(net,
        ...
        net = tf.compat.v1.estimator.layers.flatten(net)
...
def decoder(z_inp, is_training=False, getter=None, reuse=False):
    ...
    with tf.compat.v1.variable_scope('generator', reuse=reuse, custom_getter=getter):
        ...
        name_net = 'layer_3'
        with tf.compat.v1.variable_scope(name_net):
            net = tf.compat.v1.keras.layers.conv2d_transpose(net,
                                     filters=64,
                                     kernel_size=4,
                                     strides= 2,
                                     padding='same',
...
def discriminator(z_inp, x_inp, is_training=False, getter=None, reuse=False):
    ...
    with tf.compat.v1.variable_scope('discriminator', reuse=reuse, custom_getter=getter):
        ...
        name_net = 'x_layer_1'
        with tf.compat.v1.variable_scope(name_net):
            x = tf.compat.v1.keras.layers.conv2d(x_inp,
                           filters=64,
                           kernel_size=4,
                           strides=2,
                           padding='same',
            ...
            x = tf.compat.v1.layers.dropout(x, rate=0.5, name='dropout',
                                  training=is_training)

        name_net = 'x_layer_2'
        with tf.compat.v1.variable_scope(name_net):
            x = tf.compat.v1.keras.layers.conv2d(x,
                           filters=64,
                           kernel_size=4,
                           strides=2,
                           padding='same',
        ...
        with tf.compat.v1.variable_scope(name_z):
            z = tf.compat.v1.layers.dense(z_inp,
                                512,
                                kernel_initializer=init_kernel,
                                name='fc')
            z = leakyReLu(z, name='leaky_relu')
            z = tf.compat.v1.layers.dropout(z, rate=0.5, name='dropout',
                                  training=is_training)
        ...
        with tf.compat.v1.variable_scope(name_y):
            ...
            y = tf.compat.v1.layers.dropout(y, rate=0.5, name='dropout',
                                  training=is_training)
